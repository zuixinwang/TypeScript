...
namespace ts.Bumbershoot.formatting {
    export namespace SmartIndenter {
        ...
        function getIndentationForNodeWorker(
            ...
            let parent = current.parent as Node;
        ...
        export function isArgumentAndStartLineOverlapsExpressionBeingCalled(parent: Node, child: Node, childStartLine: number, sourceFile: SourceFileLike): boolean {
            if (!(isCallExpression(parent) && contains(parent.arguments, child as ts.Node))) {
                return false;
            }

            const expressionOfCallExpressionEnd = (parent.expression as any as Node).getEnd();
        ...
        export function childStartsOnTheSameLineWithElseInIfStatement(parent: Node, child: TextRangeWithKind, childStartLine: number, sourceFile: SourceFileLike): boolean {
            if (parent.kind === SyntaxKind.IfStatement && (<IfStatement><ts.Node>parent).elseStatement === child) {
            ...
            if (node.parent) {
                ...
                switch (node.parent.kind) {
                    case SyntaxKind.TypeReference:
                        return getListIfStartEndIsInListRange((<TypeReferenceNode><ts.Node>node.parent).typeArguments as any as NodeArray<Node>, node.getStart(sourceFile), end);
                    case SyntaxKind.ObjectLiteralExpression:
                        return (<ObjectLiteralExpression><ts.Node>node.parent).properties as any as NodeArray<Node>;
                    case SyntaxKind.ArrayLiteralExpression:
                        return (<ArrayLiteralExpression><ts.Node>node.parent).elements as any as NodeArray<Node>;
                    ...
                    case SyntaxKind.ConstructSignature: {
                        ...
                        return getListIfStartEndIsInListRange((<SignatureDeclaration><ts.Node>node.parent).typeParameters as any as NodeArray<Node>, start, end) ||
                            getListIfStartEndIsInListRange((<SignatureDeclaration><ts.Node>node.parent).parameters as any as NodeArray<Node>, start, end);
                    }
                    ...
                    case SyntaxKind.JSDocTemplateTag: {
                        ...
                        return getListIfStartEndIsInListRange(typeParameters as any, node.getStart(sourceFile), end);
                    }
                    case SyntaxKind.NewExpression:
                    case SyntaxKind.CallExpression: {
                        const start = node.getStart(sourceFile);
                        return getListIfStartEndIsInListRange((<any>node.parent).typeArguments, start, end) ||
                            getListIfStartEndIsInListRange((<any>node.parent).arguments, start, end);
                    }
                    case SyntaxKind.VariableDeclarationList:
                        return getListIfStartEndIsInListRange((<any>node.parent).declarations, node.getStart(sourceFile), end);
                    case SyntaxKind.NamedImports:
                    case SyntaxKind.NamedExports:
                        return getListIfStartEndIsInListRange((<any>node.parent).elements, node.getStart(sourceFile), end);
        ...
        function getLineIndentationWhenExpressionIsInMultiLine(node: Node, sourceFile: SourceFile, options: EditorSettings): number {
            ...
            if (node.parent && isCallOrNewExpression(node.parent) && (node.parent.expression as any) !== node) {
