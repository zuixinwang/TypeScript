/* @internal */
namespace ts.Bumbershoot.formatting {
    ...
    export function formatOnOpeningCurly(position: number, sourceFile: SourceFile, formatContext: FormatContext): TextChange[] {
        ...
        const curlyBraceRange = openingCurly.parent as Node;
    ...
    function findOutermostNodeWithinListLevel(node: Node | undefined) {
        ...
        while (current &&
            ...
            !isListElement(current.parent as Node, current)) {
    ...
    function isListElement(parent: Node, node: Node): boolean {
        switch (parent.kind) {
            ...
            case SyntaxKind.InterfaceDeclaration:
                return rangeContainsRange((<InterfaceDeclaration><ts.Node>parent).members, node);
            case SyntaxKind.ModuleDeclaration:
                const body = (<ModuleDeclaration><ts.Node>parent).body;
                return !!body && body.kind === SyntaxKind.ModuleBlock && rangeContainsRange(body.statements, node);
            case SyntaxKind.SourceFile:
            case SyntaxKind.Block:
            case SyntaxKind.ModuleBlock:
                return rangeContainsRange((<Block><ts.Node>parent).statements, node);
            case SyntaxKind.CatchClause:
                return rangeContainsRange((<CatchClause><ts.Node>parent).block.statements, node);
        }
    ...
    function findEnclosingNode(range: TextRange, sourceFile: SourceFile): Node {
        return find(sourceFile) as Node;

        function find(n: ts.Node): ts.Node {
            const candidate = forEachChild(n, c => startEndContainsRange((c as Node).getStart(sourceFile), c.end, range) && c);
            if (candidate) {
    ...
    function formatSpanWorker(originalRange: TextRange,
        ...
        function getFirstNonDecoratorTokenOfNode(node: Node) {
            ...
            switch (node.kind) {
                ...
                case SyntaxKind.MethodDeclaration:
                    if ((<MethodDeclaration><ts.Node>node).asteriskToken) {
                        return SyntaxKind.AsteriskToken;
                    }
                // falls through
                case SyntaxKind.PropertyDeclaration:
                case SyntaxKind.Parameter:
                    const name = getNameOfDeclaration(<Declaration><ts.Node>node);
                    if (name) {
                        return name.kind;
                    }
            }
        ...
        function processNode(node: Node, contextNode: Node, nodeStartLine: number, undecoratedNodeStartLine: number, indentation: number, delta: number) {
            ...
            forEachChild(
                node,
                child => {
                    processChildNode(child as Node, /*inheritedIndentation*/ Constants.Unknown, node, nodeDynamicIndentation, nodeStartLine, undecoratedNodeStartLine, /*isListItem*/ false);
                },
                nodes => {
                    processChildNodes(nodes as NodeArray<Node>, node, nodeStartLine, nodeDynamicIndentation);
                });
    ...
    export function getRangeOfEnclosingComment(
        ...
        if (jsdoc) tokenAtPosition = jsdoc.parent as Node;
        ...
        return commentRanges && find(commentRanges, range => rangeContainsPositionExclusive(range, position) ||
            ...
            //
                                     position === range.end && (range.kind === SyntaxKind.SingleLineCommentTrivia || position === (sourceFile as any as Node).getFullWidth()));
    }
    ...
    function getOpenTokenForList(node: Node, list: ReadonlyArray<Node>) {
        switch (node.kind) {
            ...
            case SyntaxKind.ArrowFunction:
                if (((<FunctionDeclaration><ts.Node>node).typeParameters as ReadonlyArray<ts.Node>) === list) {
                    return SyntaxKind.LessThanToken;
                }
                else if (((<FunctionDeclaration><ts.Node>node).parameters as ReadonlyArray<ts.Node>) === list) {
                    return SyntaxKind.OpenParenToken;
                }
                break;
            case SyntaxKind.CallExpression:
            case SyntaxKind.NewExpression:
                if (((<CallExpression><ts.Node>node).typeArguments as ReadonlyArray<ts.Node>) === list) {
                    return SyntaxKind.LessThanToken;
                }
                else if (((<CallExpression><ts.Node>node).arguments as ReadonlyArray<ts.Node>) === list) {
                    return SyntaxKind.OpenParenToken;
                }
                break;
            case SyntaxKind.TypeReference:
                if (((<TypeReferenceNode><ts.Node>node).typeArguments as ReadonlyArray<ts.Node>) === list) {
                    return SyntaxKind.LessThanToken;
                }
        }
