/* @internal */
namespace ts.Bumbershoot.SymbolDisplay {
    ...
    function getSymbolKindOfConstructorPropertyMethodAccessorFunctionOrVar(typeChecker: TypeChecker, symbol: Symbol, location: Node): ScriptElementKind {
        ...
        if (roots.length === 1
            ...
            && (typeChecker.getTypeOfSymbolAtLocation(symbol, location) as any).getNonNullableType().getCallSignatures().length !== 0) {
        ...
        if (flags & SymbolFlags.Property) {
            if (flags & SymbolFlags.Transient && (<any>symbol).checkFlags & CheckFlags.Synthetic) {
                // If union property is result of union of non method (property/accessors/variables), it is labeled as property
                const unionPropertyKind = forEach(typeChecker.getRootSymbols(symbol), rootSymbol => {
                    const rootSymbolFlags = (rootSymbol as any).getFlags();
                ...
                if (!unionPropertyKind) {
                    ...
                    const typeOfUnionProperty = typeChecker.getTypeOfSymbolAtLocation(symbol, location) as any;
    ...
    export function getSymbolModifiers(symbol: Symbol): string {
        const nodeModifiers = symbol && symbol.declarations && symbol.declarations.length > 0
            ? getNodeModifiers(symbol.declarations[0] as any)
    ...
    export function getSymbolDisplayPartsDocumentationAndSymbolKind(typeChecker: TypeChecker, symbol: Symbol, sourceFile: SourceFile, enclosingDeclaration: Node | undefined,
        ...
        if (symbolKind !== ScriptElementKind.unknown || symbolFlags & SymbolFlags.Class || symbolFlags & SymbolFlags.Alias) {
            ...
            type = isThisExpression ? (typeChecker.getTypeAtLocation(location) as Type) : (typeChecker.getTypeOfSymbolAtLocation(symbol.exportSymbol || symbol, location) as Type);

            if (location.parent && location.parent.kind === SyntaxKind.PropertyAccessExpression) {
                const right = (<PropertyAccessExpression>location.parent).name as any;
            ...
            if (callExpressionLike) {
                ...
                signature = typeChecker.getResolvedSignature(callExpressionLike, candidateSignatures)! as any; // TODO: GH#18217
                ...
                if (signature) {
                    if (useConstructSignatures && (symbolFlags & SymbolFlags.Class)) {
                        ...
                        addPrefixForAnyFunctionOrVar(type.symbol as Symbol, symbolKind);
                    }
                    ...
                    switch (symbolKind) {
                        ...
                        case ScriptElementKind.localVariableElement:
                            ...
                            if (!(getObjectFlags(type) & ObjectFlags.Anonymous) && type.symbol) {
                                addRange(displayParts, symbolToDisplayParts(typeChecker, type.symbol as Symbol, enclosingDeclaration, /*meaning*/ undefined, SymbolFormatFlags.AllowAnyNodeKind | SymbolFormatFlags.WriteTypeParametersOrArguments));
                ...
                if (locationIsSymbolDeclaration) {
                    ...
                    if (!typeChecker.isImplementationOfOverload(functionDeclaration)) {
                        signature = typeChecker.getSignatureFromDeclaration(functionDeclaration)! as Signature; // TODO: GH#18217
                    }
                    else {
                    ...
                    if (functionDeclaration.kind === SyntaxKind.Constructor) {
                        ...
                        addPrefixForAnyFunctionOrVar(functionDeclaration.kind === SyntaxKind.CallSignature &&
                                                     !(type.symbol.flags & SymbolFlags.TypeLiteral || type.symbol.flags & SymbolFlags.ObjectLiteral) ? (type.symbol as Symbol) : symbol, symbolKind);
                    }
        ...
        if (symbolFlags & SymbolFlags.Class && !hasAddedSymbolInfo && !isThisExpression) {
            ...
            displayParts.push(spacePart());
            addFullSymbolName(symbol);
            writeTypeParametersOfSymbol(symbol, sourceFile as any);
        }
        ...
        if (symbolFlags & SymbolFlags.TypeAlias) {
            ...
            displayParts.push(spacePart());
            addRange(displayParts, typeToDisplayParts(typeChecker, typeChecker.getDeclaredTypeOfSymbol(symbol) as Type, enclosingDeclaration, TypeFormatFlags.InTypeAlias));
        }
        ...
        if ((symbolFlags & SymbolFlags.TypeParameter) && (semanticMeaning & SemanticMeaning.Type)) {
            ...
            if (symbol.parent) {
                ...
                addInPrefix();
                addFullSymbolName(symbol.parent as Symbol, enclosingDeclaration);
                writeTypeParametersOfSymbol(symbol.parent as Symbol, enclosingDeclaration);
            }
            else {
                ...
                if (declaration) {
                    if (isFunctionLikeKind(declaration.kind)) {
                        ...
                        if (declaration.kind === SyntaxKind.ConstructSignature) {
                            ...
                            addFullSymbolName(declaration.symbol as Symbol);
                        }
                        addRange(displayParts, signatureToDisplayParts(typeChecker, signature as Signature, sourceFile as any, TypeFormatFlags.WriteTypeArgumentsOfSignature));
                    }
                        ...
                        displayParts.push(spacePart());
                        addFullSymbolName(declaration.symbol as Symbol);
                        writeTypeParametersOfSymbol(declaration.symbol as Symbol, sourceFile as any);
                    }
                }
            }
        }
                ...
                if (resolvedSymbol !== symbol && resolvedSymbol.declarations && resolvedSymbol.declarations.length > 0) {
                    ...
                    if (declarationName) {
                        ...
                        const resolvedInfo = getSymbolDisplayPartsDocumentationAndSymbolKind(
                            typeChecker,
                            resolvedSymbol as Symbol,
                            getSourceFileOfNode(resolvedNode) as any,
                            resolvedNode as any,
                            declarationName as any,
                            semanticMeaning,
                            shouldUseAliasName ? symbol : resolvedSymbol as Symbol);
                ...
                if (declaration.kind === SyntaxKind.ImportEqualsDeclaration) {
                    ...
                    if (isExternalModuleImportEqualsDeclaration(importEqualsDeclaration)) {
                        ...
                        if (internalAliasSymbol) {
                            ...
                            displayParts.push(operatorPart(SyntaxKind.EqualsToken));
                            displayParts.push(spacePart());
                            addFullSymbolName(internalAliasSymbol as Symbol, enclosingDeclaration);
                        }
                    }
                    return true;
                }
            ...
            if (documentation.length === 0 && symbolFlags & SymbolFlags.Property) {
                ...
                if (symbol.parent && forEach(symbol.parent.declarations, declaration => declaration.kind === SyntaxKind.SourceFile)) {
                    for (const declaration of symbol.declarations) {
                        ...
                        documentation = (rhsSymbol as Symbol).getDocumentationComment(typeChecker);
                        tags = (rhsSymbol as Symbol).getJsDocTags();
        ...
        function addFullSymbolName(symbolToDisplay: Symbol, enclosingDeclaration?: Node) {
            ...
            const fullSymbolDisplayParts = symbolToDisplayParts(typeChecker, symbolToDisplay, enclosingDeclaration || (sourceFile as any), /*meaning*/ undefined,
