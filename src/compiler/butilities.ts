...
namespace ts.Bumbershoot {
    ...
    export function getMeaningFromDeclaration(node: Node): SemanticMeaning {
        switch (node.kind) {
            ...
            case SyntaxKind.JSDocTypedefTag:
                ...
                return (node as any).name === undefined ? SemanticMeaning.Value | SemanticMeaning.Type : SemanticMeaning.Type;
            ...
            case SyntaxKind.ModuleDeclaration:
                if (isAmbientModule(<any>node)) {
                    return SemanticMeaning.Namespace | SemanticMeaning.Value;
                }
                else if (getModuleInstanceState(node as any) === ModuleInstanceState.Instantiated) {
                    return SemanticMeaning.Namespace | SemanticMeaning.Value;
                }
                else {
    ...
    export function getMeaningFromLocation(node: Node): SemanticMeaning {
        if (node.kind === SyntaxKind.SourceFile) {
            ...
            return getMeaningFromRightHandSideOfImportEquals(node as any);
        }
        else if (isDeclarationName(node)) {
            return getMeaningFromDeclaration(node.parent as Node);
        }
    ...
    function getMeaningFromRightHandSideOfImportEquals(node: Node): SemanticMeaning {
        ...
        const name = node.kind === SyntaxKind.QualifiedName ? node : isQualifiedName(node.parent) && (node.parent as any).right === node ? node.parent : undefined;
    ...
    export function isInRightSideOfInternalImportEqualsDeclaration(node: Node) {
        ...
        return isInternalModuleImportEqualsDeclaration(node.parent) && (node.parent as any).moduleReference === node;
    }
    ...
    function isQualifiedNameNamespaceReference(node: Node): boolean {
        ...
        if (root.parent.kind === SyntaxKind.QualifiedName) {
            while (root.parent && root.parent.kind === SyntaxKind.QualifiedName) {
                root = root.parent as any;
            }

            isLastClause = (<any>root).right === node;
        }
    ...
    function isPropertyAccessNamespaceReference(node: Node): boolean {
        ...
        if (root.parent.kind === SyntaxKind.PropertyAccessExpression) {
            ...
            isLastClause = (<any>root).name === node;
        }
    ...
    function isCallOrNewExpressionTargetWorker(node: Node, pred: (node: Node) => boolean): boolean {
        const target = climbPastPropertyAccess(node);
        return !!target && !!target.parent && pred(target.parent as any) && (target.parent as any).expression === target;
    }
    ...
    export function getTargetLabel(referenceNode: Node, labelName: string): Identifier | undefined {
        while (referenceNode) {
            if (referenceNode.kind === SyntaxKind.LabeledStatement && (<any>referenceNode).label.escapedText === labelName) {
                return (<any>referenceNode).label;
            }
            referenceNode = referenceNode.parent as Node;
        }
        return undefined;
    }

    export function isJumpStatementTarget(node: Node): node is Identifier & { parent: BreakOrContinueStatement } {
        return node.kind === SyntaxKind.Identifier && isBreakOrContinueStatement(node.parent) && (node.parent as any).label === node;
     }

    export function isLabelOfLabeledStatement(node: Node): node is Identifier {
        return node.kind === SyntaxKind.Identifier && isLabeledStatement(node.parent) && (node.parent as any).label === node;
    }
    ...
    export function isRightSideOfQualifiedName(node: Node) {
        return node.parent.kind === SyntaxKind.QualifiedName && (<any>node.parent).right === node;
    }

    export function isRightSideOfPropertyAccess(node: Node) {
        return node && node.parent && node.parent.kind === SyntaxKind.PropertyAccessExpression && (<any>node.parent).name === node;
    }

    export function isNameOfModuleDeclaration(node: Node) {
        return node.parent.kind === SyntaxKind.ModuleDeclaration && (<any>node.parent).name === node;
    }

    export function isNameOfFunctionDeclaration(node: Node): boolean {
        return node.kind === SyntaxKind.Identifier &&
            isFunctionLike(node.parent) && (<any>node.parent).name === node;
    }
    ...
    export function isExpressionOfExternalModuleImportEqualsDeclaration(node: Node) {
        return isExternalModuleImportEqualsDeclaration(node.parent.parent) &&
            getExternalModuleImportEqualsDeclarationExpression(node.parent.parent) as any === node;
    }
    ...
    export function getNodeKind(node: Node): ScriptElementKind {
        switch (node.kind) {
            case SyntaxKind.SourceFile:
                return isExternalModule(<any>node) ? ScriptElementKind.moduleElement : ScriptElementKind.scriptElement;
            case SyntaxKind.ModuleDeclaration:
            ...
            case SyntaxKind.VariableDeclaration:
                return getKindOfVariableDeclaration(<any>node);
            case SyntaxKind.BindingElement:
            ...
            case SyntaxKind.BinaryExpression:
                const kind = getSpecialPropertyAssignmentKind(node as any);
                const { right } = node as any;
                switch (kind) {
    ...
    export function isThis(node: Node): boolean {
        switch (node.kind) {
            ...
            case SyntaxKind.Identifier:
                // 'this' as a parameter
                return identifierIsThisKeyword(node as any) && node.parent.kind === SyntaxKind.Parameter;
            default:
                return false;
        }
    }
    ...
    function isCompletedNode(n: Node | undefined, sourceFile: SourceFile): boolean {
        ...
        switch (n.kind) {
            ...
            case SyntaxKind.CatchClause:
                return isCompletedNode((<any>n).block, sourceFile);
            ...
            case SyntaxKind.ConstructorType:
                return isCompletedNode((<any>n).type, sourceFile);
            ...
            case SyntaxKind.ArrowFunction:
                if ((<any>n).body) {
                    return isCompletedNode((<any>n).body, sourceFile);
                }
            ...
            case SyntaxKind.ModuleDeclaration:
                return !!(<any>n).body && isCompletedNode((<any>n).body, sourceFile);

            case SyntaxKind.IfStatement:
                if ((<any>n).elseStatement) {
                    return isCompletedNode((<any>n).elseStatement, sourceFile);
                }
                return isCompletedNode((<any>n).thenStatement, sourceFile);

            case SyntaxKind.ExpressionStatement:
                return isCompletedNode((<any>n).expression, sourceFile) ||
            ...
            case SyntaxKind.WhileStatement:
                return isCompletedNode((<any>n).statement, sourceFile);
            case SyntaxKind.DoStatement:
                ...
                return hasChildOfKind(n, SyntaxKind.WhileKeyword, sourceFile)
                    ...
                    : isCompletedNode((<any>n).statement, sourceFile);

            case SyntaxKind.TypeQuery:
                return isCompletedNode((<any>n).exprName, sourceFile);
            ...
            case SyntaxKind.SpreadElement:
                const unaryWordExpression = n as any;
                return isCompletedNode(unaryWordExpression.expression, sourceFile);

            case SyntaxKind.TaggedTemplateExpression:
                return isCompletedNode((<any>n).template, sourceFile);
            case SyntaxKind.TemplateExpression:
                const lastSpan = lastOrUndefined((<any>n).templateSpans);
                return isCompletedNode(lastSpan as any, sourceFile);
            case SyntaxKind.TemplateSpan:
                return nodeIsPresent((<any>n).literal);

            case SyntaxKind.ExportDeclaration:
            case SyntaxKind.ImportDeclaration:
                return nodeIsPresent((<any>n).moduleSpecifier);

            case SyntaxKind.PrefixUnaryExpression:
                return isCompletedNode((<any>n).operand, sourceFile);
            case SyntaxKind.BinaryExpression:
                return isCompletedNode((<any>n).right, sourceFile);
            case SyntaxKind.ConditionalExpression:
                return isCompletedNode((<any>n).whenFalse, sourceFile);
    ...
    export function findListItemInfo(node: Node): ListItemInfo | undefined {
        const list = findContainingList(node) as any;
    ...
    export function findContainingList(node: Node): SyntaxList | undefined {
        ...
        const syntaxList = find((node.parent as Node).getChildren(), c => isSyntaxList(c) && rangeContainsRange(c, node)) as any;
    ...
    export function findPrecedingToken(position: number, sourceFile: SourceFile, startNode?: Node, excludeJsdoc?: boolean): Node | undefined {
        const result = find(startNode || (sourceFile as any));
    ...
    export function isPossiblyTypeArgumentPosition(token: Node, sourceFile: SourceFile, checker: TypeChecker): boolean {
        const info = getPossibleTypeArgumentsInfo(token, sourceFile) as any;
    ...
    export function getPossibleGenericSignatures(called: Expression, typeArgumentCount: number, checker: TypeChecker): ReadonlyArray<Signature> {
        const type = checker.getTypeAtLocation(called) as any;
        const signatures = isNewExpression(called.parent) ? type.getConstructSignatures() : type.getCallSignatures();
        return signatures.filter((candidate: any) => !!candidate.typeParameters && candidate.typeParameters.length >= typeArgumentCount);
    }
            ...
            switch (token.kind) {
                case SyntaxKind.LessThanToken:
                    ...
                    if (!remainingLessThanTokens) {
                        return isDeclarationName(token) ? undefined : { called: token, nTypeArguments } as any;
                    }
    ...
    export function isInsideTemplateLiteral(node: TemplateLiteralToken, position: number, sourceFile: SourceFile): boolean {
        return isTemplateLiteralKind(node.kind)
            && ((node as any).getStart(sourceFile) < position && position < node.end) || (!!node.isUnterminated && position === node.end);
    }
    ...
    export function isArrayLiteralOrObjectLiteralDestructuringPattern(node: Node) {
        if (node.kind === SyntaxKind.ArrayLiteralExpression ||
            ...
            if (node.parent.kind === SyntaxKind.BinaryExpression &&
                (<any>node.parent).left === node &&
                (<any>node.parent).operatorToken.kind === SyntaxKind.EqualsToken) {
                return true;
            }
            ...
            if (node.parent.kind === SyntaxKind.ForOfStatement &&
                (<any>node.parent).initializer === node) {
                return true;
            }
            ...
            if (isArrayLiteralOrObjectLiteralDestructuringPattern((node.parent.kind === SyntaxKind.PropertyAssignment ? node.parent.parent : node.parent) as any)) {
                return true;
            }
        }
    ...
    export function makeImport(defaultImport: Identifier | undefined, namedImports: ReadonlyArray<ImportSpecifier> | undefined, moduleSpecifier: string | Expression, quotePreference: QuotePreference): ImportDeclaration {
        return createImportDeclaration(
            ...
            defaultImport || namedImports
                ? createImportClause(defaultImport as any, namedImports && namedImports.length ? createNamedImports(namedImports) : undefined)
                : undefined,
    ...
    export function getPropertySymbolsFromBaseTypes<T>(symbol: Symbol, propertyName: string, checker: TypeChecker, cb: (symbol: Symbol) => T | undefined): T | undefined {
        ...
        function recur(symbol: Symbol): T | undefined {
            ...
            return firstDefined(symbol.declarations, declaration => firstDefined(getAllSuperTypeNodes(declaration), typeReference => {
                ...
                return type && propertySymbol && (firstDefined(checker.getRootSymbols(propertySymbol), cb) || recur(type.symbol as any));
            }));
        }
    }

    export function isMemberSymbolInBaseType(memberSymbol: Symbol, checker: TypeChecker): boolean {
        return getPropertySymbolsFromBaseTypes(memberSymbol.parent as any, memberSymbol.name, checker, _ => true) || false;
    }
    ...
    export function getParentNodeInSpan(node: Node | undefined, file: SourceFile, span: TextSpan): Node | undefined {
        ...
        while (node.parent) {
            if (isSourceFile(node.parent) || !spanContainsNode(span, node.parent as Node, file)) {
                return node;
            }
...
namespace ts.Bumbershoot {
    ...
    export function isImportOrExportSpecifierName(location: Node): location is Identifier {
        return !!location.parent &&
            ...
            (<any>location.parent).propertyName === location;
    }
    ...
    function getSynthesizedDeepCloneWorker<T extends Node>(node: T): T {
        const visited = visitEachChild(node, getSynthesizedDeepClone as any, nullTransformationContext);
        if (visited === node) {
    ...
    export function suppressTrailingTrivia(node: Node) {
        addEmitFlagsRecursively(node, EmitFlags.NoTrailingComments, getLastChild as any);
    }
