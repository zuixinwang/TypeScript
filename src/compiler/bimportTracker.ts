...
namespace ts.Bumbershoot.FindAllReferences {
    ...
    function getImportersForExport(
        ...
        const markSeenDirectImport = nodeSeenTracker<any>();
        const markSeenIndirectUser = nodeSeenTracker<any>();
        ...
        function getIndirectUsers(): ReadonlyArray<SourceFile> {
            ...
            for (const decl of exportingModuleSymbol.declarations) {
                if (isExternalModuleAugmentation(decl) && sourceFilesSet.has((decl as any).getSourceFile().fileName)) {
            ...
            return indirectUserDeclarations!.map<SourceFile>(getSourceFileOfNode as any);
        }
                ...
                for (const direct of theseDirectImports) {
                    ...
                    switch (direct.kind) {
                        ...
                        case SyntaxKind.ImportEqualsDeclaration:
                            handleNamespaceImport(direct as any, (direct as any).name, hasModifier(direct, ModifierFlags.Export), /*alreadyAddedDirect*/ false);
                            break;

                        case SyntaxKind.ImportDeclaration:
                            directImports.push(direct);
                            const namedBindings = (direct as any).importClause && (direct as any).importClause.namedBindings;
                            if (namedBindings && namedBindings.kind === SyntaxKind.NamespaceImport) {
                                handleNamespaceImport((direct as any), namedBindings.name, /*isReExport*/ false, /*alreadyAddedDirect*/ true);
                            }
                            else if (!isAvailableThroughGlobal && isDefaultImport((direct as any))) {
                        ...
                        default:
                            Debug.assertNever((direct as never), `Unexpected import kind: ${Debug.showSyntaxKind(direct)}`);
                    }
                }
            }
        }
        ...
        function addIndirectUsers(sourceFileLike: SourceFileLike): void {
            ...
            const directImports = getDirectImports(moduleSymbol as Symbol);
            if (directImports) {
    ...
    function getSearchesFromDirectImports(directImports: Importer[], exportSymbol: Symbol, exportKind: ExportKind, checker: TypeChecker, isForRename: boolean): Pick<ImportsResult, "importSearches" | "singleReferences"> {
        ...
        function handleImport(decl: any): void {
            if (decl.kind === SyntaxKind.ImportEqualsDeclaration) {
                if (isExternalModuleImportEquals(decl)) {
                    handleNamespaceImportLike(decl.name as any);
                }
                return;
            }
            ...
            if (namedBindings) {
                switch (namedBindings.kind) {
                    ...
                    case SyntaxKind.NamedImports:
                        ...
                        break;
                    default:
                        Debug.assertNever(namedBindings as never);
                }
            }
            ...
            if (name && (exportKind === ExportKind.Default || exportKind === ExportKind.ExportEquals) && (!isForRename || name.escapedText === symbolEscapedNameNoDefault(exportSymbol))) {
                ...
                addSearch(name, defaultImportAlias as Symbol);
            }
        }
         ...
         */
        function handleNamespaceImportLike(importName: any): void {
            // Don't rename an import that already has a different name than the export.
            if (exportKind === ExportKind.ExportEquals && (!isForRename || isNameMatch(importName.escapedText))) {
                addSearch(importName, checker.getSymbolAtLocation(importName)! as Symbol);
            }
        }
        ...
        function searchForNamedImport(namedBindings: NamedImportsOrExports | undefined): void {
            ...
            for (const element of namedBindings.elements) {
                ...
                if (propertyName) {
                    ...
                    singleReferences.push(propertyName as any);
                    ...
                    if (!isForRename || name.escapedText === exportSymbol.escapedName) {
                        ...
                        addSearch(name as any, checker.getSymbolAtLocation(name)! as Symbol);
                    }
                }
                else {
                    ...
                    addSearch(name as any, localSymbol as Symbol);
                }
            }
        }
    ...
    function forEachPossibleImportOrExportStatement<T>(sourceFileLike: SourceFileLike, action: (statement: Statement) => T): T | undefined {
        return forEach(sourceFileLike.kind === SyntaxKind.SourceFile ? sourceFileLike.statements : sourceFileLike.body!.statements, (statement: any) => // TODO: GH#18217
     ...
     */
    export function getImportOrExportSymbol(node: Node, symbol: any, checker: TypeChecker, comingFromExport: boolean): ImportedSymbol | ExportedSymbol | undefined {
        ...
        function getExport(): ExportedSymbol | ImportedSymbol | undefined {
            ...
            if (symbol.exportSymbol) {
                if (parent.kind === SyntaxKind.PropertyAccessExpression) {
                    ...
                    return symbol.declarations.some((d: any) => d === parent) && isBinaryExpression(grandParent)
                        ? getSpecialPropertyExport(grandParent, /*useLhsSymbol*/ false)
                        : undefined;
                }
                else {
                    return exportInfo(symbol.exportSymbol, getExportKindForDeclaration(parent as Node));
                }
            }
            else {
                const exportNode = getExportNode(parent as Node, node);
                if (exportNode && hasModifier(exportNode, ModifierFlags.Export)) {
                    if (isImportEqualsDeclaration(exportNode) && exportNode.moduleReference as any === node) {
                        ...
                        return { kind: ImportExport.Import, symbol: lhsSymbol as Symbol, isNamedImport: false };
                    }
                    else {
            ...
            function getExportAssignmentExport(ex: ExportAssignment): ExportedSymbol {
                ...
                const exportingModuleSymbol = Debug.assertDefined(ex.symbol.parent, "Expected export symbol to have a parent") as any;
        ...
        function getImport(): ImportedSymbol | undefined {
            ...
            let importedSymbol = checker.getImmediateAliasedSymbol(symbol) as Symbol;
            ...
            if (importedSymbol.escapedName === "export=") {
                importedSymbol = getExportEqualsLocalSymbol(importedSymbol, checker) as Symbol;
            }
    ...
    function getExportEqualsLocalSymbol(importedSymbol: Symbol, checker: TypeChecker): ts.Symbol {
    ...
    function isNodeImport(node: Node): { isNamedImport: boolean } | undefined {
        ...
        switch (parent.kind) {
            case SyntaxKind.ImportEqualsDeclaration:
                return (parent as any).name === node && isExternalModuleImportEquals(parent as ImportEqualsDeclaration)
                    ? { isNamedImport: false }
                    : undefined;
            ...
            case SyntaxKind.NamespaceImport:
                Debug.assert((parent as any).name === node);
    ...
    export function getExportInfo(exportSymbol: Symbol, exportKind: ExportKind, checker: TypeChecker): ExportInfo | undefined {
        ...
        const exportingModuleSymbol = checker.getMergedSymbol(moduleSymbol) as Symbol; // Need to get merged symbol in case there's an augmentation.
        ...
        if (symbol.declarations) {
            for (const declaration of symbol.declarations) {
                if (isExportSpecifier(declaration) && !declaration.propertyName && !declaration.parent.parent.moduleSpecifier) {
                    return checker.getExportSpecifierLocalTargetSymbol(declaration)! as Symbol;
                }
            }
        }
        return symbol;
    ...
    function getContainingModuleSymbol(importer: Importer, checker: TypeChecker): Symbol {
        return checker.getMergedSymbol(getSourceFileLikeForImportDeclaration(importer).symbol) as Symbol;
    }

    function getSourceFileLikeForImportDeclaration(node: ImporterOrCallExpression): SourceFileLike {
        if (node.kind === SyntaxKind.CallExpression) {
            return (node as any).getSourceFile();
        }
