/* @internal */
namespace ts.Bumbershoot.FindAllReferences {
    ...
    export function nodeEntry(node: any, isInString?: true): NodeEntry {
    ...
    function definitionToReferencedSymbolDefinitionInfo(def: Definition, checker: TypeChecker, originalNode: Node): ReferencedSymbolDefinitionInfo {
            ...
            switch (def.type) {
                ...
                case "this": {
                    ...
                    const displayParts = symbol && SymbolDisplay.getSymbolDisplayPartsDocumentationAndSymbolKind(
                        checker, symbol as Symbol, node.getSourceFile(), getContainerNode(node) as any, node).displayParts || [textPart("this")];
        ...
        return { containerKind: ScriptElementKind.unknown, containerName: "", fileName: sourceFile.fileName, kind, name, textSpan: getTextSpan((isComputedPropertyName(node as Node) ? (node as ts.ComputedPropertyName).expression : node) as any, sourceFile), displayParts };
    }
    ...
    function getDefinitionKindAndDisplayParts(symbol: Symbol, checker: TypeChecker, node: Node): { displayParts: SymbolDisplayPart[], kind: ScriptElementKind } {
        ...
        const enclosingDeclaration = (firstOrUndefined(symbol.declarations) || node) as any;
    ...
    function implementationKindDisplayParts(node: Node, checker: TypeChecker): { kind: ScriptElementKind, displayParts: SymbolDisplayPart[] } {
        ...
        if (symbol) {
            return getDefinitionKindAndDisplayParts(symbol as Symbol, checker, node);
        }
...
/* @internal */
namespace ts.Bumbershoot.FindAllReferences.Core {
    ...
    export function getReferencedSymbolsForNode(position: number, node: Node, program: Program, sourceFiles: ReadonlyArray<SourceFile>, cancellationToken: CancellationToken, options: Options = {}, sourceFilesSet: ReadonlyMap<true> = arrayToSet(sourceFiles, f => f.fileName)): SymbolAndEntries[] | undefined {
        if (isSourceFile(node)) {
            const reference = GoToDefinition.getReferenceAtPosition(node as any, position, program);
            const moduleSymbol = reference && program.getTypeChecker().getMergedSymbol(reference.file.symbol);
            return moduleSymbol && getReferencedSymbolsForModule(program, moduleSymbol as any, /*excludeImportTypeOfExportEquals*/ false, sourceFiles, sourceFilesSet);
        }
        ...
        const checker = program.getTypeChecker();
        let symbol = checker.getSymbolAtLocation(node) as Symbol;
        ...
        if (moduleSourceFile) {
            ...
            symbol = skipAlias(exportEquals, checker) as Symbol;
    ...
    function isModuleSymbol(symbol: Symbol): SourceFile | undefined {
        return symbol.flags & SymbolFlags.Module ? find(symbol.declarations, isSourceFile) as SourceFile : undefined;
    }
    ...
    function getReferencedSymbolsForModule(program: Program, symbol: Symbol, excludeImportTypeOfExportEquals: boolean, sourceFiles: ReadonlyArray<SourceFile>, sourceFilesSet: ReadonlyMap<true>): SymbolAndEntries[] {
        ...
        const references = mapDefined<ModuleReference, Entry>(ts.Bumbershoot.FindAllReferences.findModuleReferences(program, sourceFiles, symbol), reference => {
            if (reference.kind === "import") {
                ...
                return { type: "node", node: reference.literal as ts.Node as Node };
            }
            else {
                return {
                    type: "span",
        ...
        for (const decl of symbol.declarations) {
            switch (decl.kind) {
                ...
                case SyntaxKind.ModuleDeclaration:
                    if (sourceFilesSet.has((decl as ts.Node as Node).getSourceFile().fileName)) {
                        references.push({ type: "node", node: (decl as ModuleDeclaration).name as ts.Node as Node });
                    }
                    break;
                default:
    ...
    function getReferencedSymbolsSpecial(node: Node, sourceFiles: ReadonlyArray<SourceFile>, cancellationToken: CancellationToken): SymbolAndEntries[] | undefined {
        ...
        if (isJumpStatementTarget(node)) {
            const labelDefinition = getTargetLabel(node.parent as any, node.text);
            // if we have a label definition, look within its statement for references, if not, then
            // the label is undefined and we have no results..
            return labelDefinition && getLabelReferencesInNode(labelDefinition.parent as Node, labelDefinition);
        }
        else if (isLabelOfLabeledStatement(node)) {
            // it is a label definition and not a target, search within the parent labeledStatement
            return getLabelReferencesInNode(node.parent as Node, node);
        }
    ...
    function getReferencedSymbolsForSymbol(symbol: Symbol, node: Node | undefined, sourceFiles: ReadonlyArray<SourceFile>, sourceFilesSet: ReadonlyMap<true>, checker: TypeChecker, cancellationToken: CancellationToken, options: Options): SymbolAndEntries[] {
        ...
        if (node && node.kind === SyntaxKind.DefaultKeyword) {
            ...
            searchForImportsOfExport(node, symbol, { exportingModuleSymbol: Debug.assertDefined(symbol.parent, "Expected export symbol to have a parent") as Symbol, exportKind: ExportKind.Default }, state);
        }
        else {
            ...
            if (scope) {
                getReferencesInContainer(scope, scope.getSourceFile(), search, state, /*addReferencesHere*/ !(isSourceFile(scope) && !contains(sourceFiles, scope as any)));
            }
            else {
                // Global search
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.Identifier:
                if (isClassLike(node.parent)) {
                    Debug.assert(node.parent.name as ts.Node as Node === node);
    ...
    function skipPastExportOrImportSpecifierOrUnion(symbol: Symbol, node: Node, checker: TypeChecker): Symbol | undefined {
        ...
        if (isImportSpecifier(parent) && parent.propertyName === node as any) {
            // We're at `foo` in `import { foo as bar }`. Probably intended to find all refs on the original, not just on the import.
            return checker.getImmediateAliasedSymbol(symbol)! as Symbol;
        }
    ...
    function searchForImportsOfExport(exportLocation: Node, exportSymbol: Symbol, exportInfo: ExportInfo, state: State): void {
        ...
        if (singleReferences.length) {
            ...
            for (const singleRef of singleReferences) {
                if (shouldAddSingleReference(singleRef, state)) addRef(singleRef as ts.Node as Node);
            }
        }
    ...
    function shouldAddSingleReference(singleRef: Identifier | StringLiteral, state: State): boolean {
        if (!hasMatchingMeaning(singleRef as Node, state)) return false;
    ...
    function searchForImportedSymbol(symbol: Symbol, state: State): void {
        for (const declaration of symbol.declarations) {
            const exportingFile = (declaration as any).getSourceFile();
            // Need to search in the file even if it's not in the search-file set, because it might export the symbol.
            getReferencesInSourceFile(exportingFile, state.createSearch((declaration as any), symbol, ImportExport.Import), state, state.includesSourceFile(exportingFile));
        }
    }
    ...
    function getPropertySymbolOfDestructuringAssignment(location: Node, checker: TypeChecker): Symbol | undefined {
        return isArrayLiteralOrObjectLiteralDestructuringPattern(location.parent.parent as Node)
            ? checker.getPropertySymbolOfDestructuringAssignment(<any>location) as Symbol
            : undefined;
    }
    ...
    function getPropertySymbolOfObjectBindingPatternWithoutPropertyName(symbol: Symbol, checker: TypeChecker): Symbol | undefined {
        ...
        return bindingElement && getPropertySymbolFromBindingElement(checker, bindingElement) as Symbol;
    }
    ...
    function getSymbolScope(symbol: Symbol): Node | undefined {
        ...
        if (flags & (SymbolFlags.Property | SymbolFlags.Method)) {
            ...
            if (privateDeclaration) {
                return getAncestor(privateDeclaration, SyntaxKind.ClassDeclaration) as Node | undefined;
            }
        ...
        */
        ...
        if (exposedByParent && !(isExternalModuleSymbol(parent! as Symbol) && !parent!.globalExports)) {
            return undefined;
        }

        let scope: Node | undefined;
        for (const declaration of declarations) {
            const container = getContainerNode(declaration as any);
            if (scope && scope !== container as any) {
        ...
        return exposedByParent ? (scope!.getSourceFile() as any) : scope; // TODO: GH#18217
    }
    ...
    export function eachSymbolReferenceInFile<T>(definition: Identifier, checker: TypeChecker, sourceFile: SourceFile, cb: (token: Identifier) => T): T | undefined {
        const symbol = checker.getSymbolAtLocation(definition) as Symbol;
        if (!symbol) return undefined;
        for (const token of getPossibleSymbolReferenceNodes(sourceFile, symbol.name)) {
            if (!isIdentifier(token) || token as any === definition || token.escapedText !== (definition as any).escapedText) continue;
            const referenceSymbol: Symbol = checker.getSymbolAtLocation(token)! as Symbol; // See GH#19955 for why the type annotation is necessary
            if (referenceSymbol === symbol
                || checker.getShorthandAssignmentValueSymbol(token.parent) === symbol
                || isExportSpecifier(token.parent) && getLocalSymbolForExportSpecifier(token as any, referenceSymbol, token.parent, checker) === symbol) {
                const res = cb(token as any);
                if (res) return res;
            }
        }
    }
    ...
    export function eachSignatureCall(signature: SignatureDeclaration, sourceFiles: ReadonlyArray<SourceFile>, checker: TypeChecker, cb: (call: CallExpression) => void): void {
        ...
        const symbol = Debug.assertDefined(checker.getSymbolAtLocation(signature.name)) as Symbol;
    ...
    function getPossibleSymbolReferenceNodes(sourceFile: SourceFile, symbolName: string, container: any = sourceFile): ReadonlyArray<Node> {
    ...
    function getPossibleSymbolReferencePositions(sourceFile: SourceFile, symbolName: string, container: any = sourceFile): ReadonlyArray<number> {
    ...
    function isValidReferencePosition(node: Node, searchSymbolName: string): boolean {
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.NumericLiteral:
                return isLiteralNameOfPropertyDeclarationOrIndexAccess(node as any) && (node as any).text.length === searchSymbolName.length;
    ...
    function getReferencesInSourceFile(sourceFile: any, search: Search, state: State, addReferencesHere = true): void {
        state.cancellationToken.throwIfCancellationRequested();
    ...
    function getReferencesAtLocation(sourceFile: SourceFile, position: number, search: Search, state: State, addReferencesHere: boolean): void {
        ...
        const referenceSymbol = state.checker.getSymbolAtLocation(referenceLocation) as Symbol;
        if (!referenceSymbol) {
            return;
        }

        const parent = referenceLocation.parent;
        if (isImportSpecifier(parent) && parent.propertyName === referenceLocation as ts.Node) {
    ...
    function getReferencesAtExportSpecifier(referenceLocation: Identifier, referenceSymbol: Symbol, exportSpecifier: ExportSpecifier, search: Search, state: State, addReferencesHere: boolean): void {
        ...
        if (!propertyName) {
        ...
        else if (referenceLocation === propertyName as ts.Node as Identifier) {
            ...
            if (addReferencesHere && !state.options.isForRename && state.markSeenReExportRHS(name as any)) {
                addReference(name as any, referenceSymbol, state);
            }
        }
        else {
        ...
        if (!(referenceLocation === propertyName as ts.Node as Identifier && state.options.isForRename)) {
            const exportKind = (referenceLocation as any).originalKeywordKind === SyntaxKind.DefaultKeyword ? ExportKind.Default : ExportKind.Named;
        ...
        if (search.comingFrom !== ImportExport.Export && exportDeclaration.moduleSpecifier && !propertyName) {
            ...
            if (imported) searchForImportedSymbol(imported as Symbol, state);
        }
    ...
    function getLocalSymbolForExportSpecifier(referenceLocation: Identifier, referenceSymbol: Symbol, exportSpecifier: ExportSpecifier, checker: TypeChecker): Symbol {
        return isExportSpecifierAlias(referenceLocation, exportSpecifier) && checker.getExportSpecifierLocalTargetSymbol(exportSpecifier) as Symbol || referenceSymbol;
    }

    function isExportSpecifierAlias(referenceLocation: Identifier, exportSpecifier: ExportSpecifier): boolean {
        const { parent, propertyName, name } = exportSpecifier;
        Debug.assert(propertyName as any === referenceLocation || name as any === referenceLocation);
        if (propertyName) {
            // Given `export { foo as bar } [from "someModule"]`: It's an alias at `foo`, but at `bar` it's a new symbol.
            return propertyName as any === referenceLocation;
        }
        else {
    ...
    function getReferenceForShorthandProperty({ flags, valueDeclaration }: Symbol, search: Search, state: State): void {
        const shorthandValueSymbol = state.checker.getShorthandAssignmentValueSymbol(valueDeclaration)! as Symbol;
        const name = valueDeclaration && getNameOfDeclaration(valueDeclaration) as any;
        /*
    ...
    function addConstructorReferences(referenceLocation: Node, sourceFile: SourceFile, search: Search, state: State): void {
        ...
        if (isClassLike(referenceLocation.parent)) {
            Debug.assert(referenceLocation.kind === SyntaxKind.DefaultKeyword || referenceLocation.parent.name as any === referenceLocation);
    ...
    function addClassStaticThisReferences(referenceLocation: Node, search: Search, state: State): void {
        ...
        Debug.assert(classLike.name as any === referenceLocation);
        ...
        for (const member of classLike.members) {
            ...
            if (member.body) {
                (member.body as ts.Node as Node).forEachChild(function cb(node) {
    ...
    function findOwnConstructorReferences(classSymbol: Symbol, sourceFile: SourceFile, addNode: (node: Node) => void): void {
        for (const decl of classSymbol.members!.get(InternalSymbolName.Constructor)!.declarations) {
            const ctrKeyword = findChildOfKind(decl as any, SyntaxKind.ConstructorKeyword, sourceFile)!;
        ...
        classSymbol.exports!.forEach(member => {
            ...
            if (decl && decl.kind === SyntaxKind.MethodDeclaration) {
                ...
                if (body) {
                    forEachDescendantOfKind(body as ts.Node as Node, SyntaxKind.ThisKeyword, thisKeyword => {
    ...
    function findSuperConstructorAccesses(cls: ClassLikeDeclaration, addNode: (node: Node) => void): void {
        ...
        for (const decl of ctr.declarations) {
            ...
            if (body) {
                forEachDescendantOfKind(body as ts.Node as Node, SyntaxKind.SuperKeyword, node => {
    ...
    function addImplementationReferences(refNode: Node, addReference: (node: Node) => void, state: State): void {
        ...
        if (isDeclarationName(refNode) && isImplementation(refNode.parent as Node)) {
        ...
        const containingClass = getContainingClassIfInHeritageClause(refNode) as any;
        if (containingClass) {
        ...
        if (hasType(typeHavingNode) && typeHavingNode.type === typeNode && state.markSeenContainingTypeReference(typeHavingNode as ts.Node as Node)) {
        ...
        function addIfImplementation(e: Expression): void {
            if (isImplementationExpression(e)) addReference(e as ts.Node as Node);
        }
    }

    function getContainingClassIfInHeritageClause(node: Node): ClassLikeDeclaration | InterfaceDeclaration | undefined {
        return isIdentifier(node) || isPropertyAccessExpression(node) ? getContainingClassIfInHeritageClause(node.parent as Node)
    ...
    function explicitlyInheritsFrom(symbol: Symbol, parent: Symbol, cachedResults: Map<boolean>, checker: TypeChecker): boolean {
        ...
        const inherits = symbol.declarations.some(declaration =>
            getAllSuperTypeNodes(declaration).some(typeReference => {
                ...
                return !!type && !!type.symbol && explicitlyInheritsFrom(type.symbol as Symbol, parent, cachedResults, checker);
            }));
    ...
    function getReferencesForSuperKeyword(superKeyword: Node): SymbolAndEntries[] | undefined {
        let searchSpaceNode = getSuperContainer(superKeyword, /*stopOnFunctions*/ false) as Node;
        if (!searchSpaceNode) {
            return undefined;
        }
        ...
        switch (searchSpaceNode.kind) {
            ...
            case SyntaxKind.SetAccessor:
                ...
                searchSpaceNode = searchSpaceNode.parent as Node; // re-assign to be the owning class
                break;
            default:
                return undefined;
        }
        ...
        return [{ definition: { type: "symbol", symbol: searchSpaceNode.symbol as Symbol }, references }];
    }
    ...
    function getReferencesForThisKeyword(thisOrSuperKeyword: Node, sourceFiles: ReadonlyArray<SourceFile>, cancellationToken: CancellationToken): SymbolAndEntries[] | undefined {
        let searchSpaceNode = getThisContainer(thisOrSuperKeyword, /* includeArrowFunctions */ false) as Node;
        ...
        switch (searchSpaceNode.kind) {
            ...
            case SyntaxKind.SetAccessor:
                ...
                break;
            case SyntaxKind.SourceFile:
                if (isExternalModule(<any>searchSpaceNode)) {
                    return undefined;
                }
                // falls through
    ...
    function forEachRelatedSymbol<T>(
        ...
        if (containingObjectLiteralElement) {
            ...
            const contextualType = checker.getContextualType(containingObjectLiteralElement.parent) as Type;
             ...
             */
            const shorthandValueSymbol = checker.getShorthandAssignmentValueSymbol(location.parent) as Symbol;
        ...
        if (symbol.valueDeclaration && isParameterPropertyDeclaration(symbol.valueDeclaration)) {
            ...
            return fromRoot(symbol.flags & SymbolFlags.FunctionScopedVariable ? paramProps[1] as Symbol : paramProps[0] as Symbol);
        }
        ...
        function fromRoot(sym: Symbol): T | undefined {
            ...
            return firstDefined(checker.getRootSymbols(sym), (rootSymbol: Symbol) =>
                ...
                || (rootSymbol.parent && rootSymbol.parent.flags & (SymbolFlags.Class | SymbolFlags.Interface) && allowBaseTypes(rootSymbol)
                    ? getPropertySymbolsFromBaseTypes(rootSymbol.parent as Symbol, rootSymbol.name, checker, base => cbSymbol(sym, rootSymbol, base))
                    : undefined));
        }
    }
    ...
    function getRelatedSymbol(search: Search, referenceSymbol: Symbol, referenceLocation: Node, state: State): Symbol | undefined {
        ...
        return forEachRelatedSymbol(referenceSymbol, referenceLocation, checker,
            ...
            /*allowBaseTypes*/ rootSymbol =>
                !(search.parents && !search.parents.some(parent => explicitlyInheritsFrom(rootSymbol.parent! as Symbol, parent, state.inheritsFromCache, checker))));
    }
    ...
    export function getIntersectingMeaningFromDeclarations(node: Node, symbol: Symbol): SemanticMeaning {
        ...
        if (declarations) {
            ...
            do {
                ...
                for (const declaration of declarations) {
                    const declarationMeaning = getMeaningFromDeclaration(declaration as any);
    ...
    export function getReferenceEntriesForShorthandPropertyAssignment(node: Node, checker: TypeChecker, addReference: (node: Node) => void): void {
        ...
        const shorthandSymbol = checker.getShorthandAssignmentValueSymbol(refSymbol.valueDeclaration) as any;
    ...
    function forEachDescendantOfKind(node: Node, kind: SyntaxKind, action: (node: Node) => void): void {
        forEachChild(node, child => {
            if (child.kind === kind) {
                action(child as Node);
            }
            forEachDescendantOfKind(child as Node, kind, action);
        });
    }
    ...
    function getParentSymbolsOfPropertyAccess(location: Node, symbol: Symbol, checker: TypeChecker): ReadonlyArray<Symbol> | undefined {
        ...
        const lhsType = propertyAccessExpression && checker.getTypeAtLocation(propertyAccessExpression.expression) as Type;
        const res = mapDefined(lhsType && (lhsType.isUnionOrIntersection() ? lhsType.types : lhsType.symbol === symbol.parent ? undefined : [lhsType]), t =>
            t.symbol && t.symbol.flags & (SymbolFlags.Class | SymbolFlags.Interface) ? t.symbol as Symbol : undefined);
